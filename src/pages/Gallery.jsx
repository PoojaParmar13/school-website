...
const GalleryPage = () => {
  ...
  return (
    <div className="py-8 px-8 pb-8 pt-[7rem] bg-gray-200 border-gray-800 border-4 relative">
      <div className="flex flex-col md:flex-row justify-center md:justify-between items-center mb-12">
        <h2 className="text-3xl font-bold md:ml-[20rem] text-gray-600 underline text-center md:mb-0 md:flex-grow">
          Our School Gallery
        </h2>
        <div className="flex flex-col md:flex-row items-center md:items-start md:ml-8 w-full md:w-auto">
          <div className="flex flex-col md:flex-row items-center md:items-start w-full md:w-auto md:ml-4">
            ...
            <select 
              ...
              className="border-gray-600 -mb-[2rem] md:-mb-[0.5rem] border-2 px-2 ml-[5rem]  md:ml-0 md:mt-0 -mt-[1.5rem] md:mr-2 py-[0.1rem] rounded-md w-50 md:w-auto"
            >
      ...
      <div className="clear-both">
        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-1">
          {filteredImages.slice(0, visibleImages).map((image, index) => (
            <div key={index} className="flex flex-col items-center rounded-md border-gray-600 border-2">
              <img 
                src={image.src} 
                alt={image.name} 
        ...
        {visibleImages < filteredImages.length && (
          <div className="text-center mt-8">
            <button 
              ...
              className="px-8 py-2 bg-blue-500 text-white rounded-lg shadow-md hover:bg-blue-600 transition duration-300"
            >
              Show More
            </button>
          </div>
